<?php
	...
	if(isset($_POST['svs_submit_lm'])){
		$del_q = "delete from ".$wpdb->prefix."svisitor_stat where vtime < DATE_SUB(NOW() , 1 MONTH)";
	...
	elseif(isset($_POST['svs_submit_all'])){
		$del_q = "delete from ".$wpdb->prefix."svisitor_stat";
	...
	$q_total = "SELECT count( * ) as total FROM ".$wpdb->prefix."svisitor_stat";
	...
	$query = "select * from ".$wpdb->prefix."svisitor_stat order by vtime desc limit $l,30";
...
<div class="wrap">
 ...
 <?php endif;?>
  ...
  <div class="tablenav top">
    <div class="tablenav-pages"> 
        ...
        <span class="pagination-links"> 
        	<a href="?page=smpv-stat&pg=1">«</a> 
            <a href="?page=smpv-stat&pg=<?php if(isset($_GET['pg']) && $_GET['pg'] > 1){$pg_p = $_GET['pg'] - 1;echo $pg_p;}else{echo 1;}?>">‹</a> 
            ...
            <a href="?page=smpv-stat&pg=<?php if(isset($_GET['pg'])){if($_GET['pg'] >= $num_page){echo $num_page;}else{$pg_n = $_GET['pg'] + 1;echo $pg_n;}}else{if($num_page > 1){echo 2;}else{echo 1;}}?>">›</a> 
            <a href="?page=smpv-stat&pg=<?php echo $num_page;?>">»</a>
       </span> 
    </div>
  </div>
  ...
  <form action="" method="post" onsubmit="return false" id="del-frm">
  	<input type="submit" name="svs_submit_lm" value="Delete Last month data" class="button-primary del-data" />
    <span style="margin-left:20px;"><input type="submit" name="svs_submit_all" value="Delete All data" class="button-primary del-data" /></span>
  </form>
  </p>
</div>
<script>
